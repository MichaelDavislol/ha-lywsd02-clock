...
### Fixed
- `NameError: name '_LOGGER' is not defined` on import, because the
  monkey-patch helper was invoked before `_LOGGER` was assigned. Moved
  the `_LOGGER` binding and the homeassistant/const imports above the
  monkey-patch helper so the order is correct.
...
### Fixed
- `bleak write error: 'str' object has no attribute 'obj'`. The
  low-level `BleakClientBlueZDBus` backend client requires
  `BleakGATTCharacteristic` *objects* for `write_gatt_char`, unlike the
  high-level `BleakClient` facade which accepts UUID strings and
  resolves them internally. We now resolve UUID → characteristic via
  the client's service tree before writing. Applies to both the
  `bluetoothctl+dbus` path and the bluezdbus-direct path.
